    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-md rounded-2xl border border-slate-800 bg-slate-950 p-6 shadow-2xl">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <h3 className="text-base font-bold text-white">Unlock Enterprise ESPs</h3>
            <p className="text-xs text-slate-400">Enter your Email Connector Subscription ID</p>
          </div>
        </div>
        ...
        <form onSubmit={handleVerify} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-white focus:border-amber-500 focus:outline-none font-mono"
            />
            <p className="text-[11px] text-slate-500 mt-1">
              Tip: Use <code className="text-amber-400 font-mono">ec_test_enterprise</code> for instant offline test validation.
            </p>
          </div>
          ...
          {status && (
            ...
            >
              {status.success ? <CheckCircle2 className="h-4 w-4 text-emerald-400 shrink-0" /> : <AlertCircle className="h-4 w-4 text-red-400 shrink-0" />}
