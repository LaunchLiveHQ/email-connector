...
export class JetEmailDriver implements EmailDriver {
  readonly id = "jetemail";
  readonly name = "JetEmail";
  ...
  constructor(credentials: DriverCredentials) {
    if (!credentials.apiKey && !credentials.serverToken) {
      throw new Error("JetEmailDriver requires an apiKey or serverToken.");
    }
  ...
  async send(message: UnifiedEmailPayload): Promise<SendResult> {
    ...
    const endpoint = "https://api.jetemail.com/email";
    ...
    if (!res.ok) {
      ...
      throw new Error(`JetEmail API failed with status ${res.status}: ${errBody}`);
    }

    const data = (await res.json()) as { id?: string };
    const messageId = data?.id || `jet_${Date.now()}`;
