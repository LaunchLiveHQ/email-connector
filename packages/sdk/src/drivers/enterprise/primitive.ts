...
export class PrimitiveDriver implements EmailDriver {
  ...
  constructor(credentials: DriverCredentials) {
    if (!credentials.apiKey && !credentials.serverToken) {
      throw new Error("PrimitiveDriver requires an apiKey or serverToken.");
    }
  ...
  async send(message: UnifiedEmailPayload): Promise<SendResult> {
    ...
    const endpoint = "https://api.primitive.io/v1/send";
    ...
    if (!res.ok) {
      ...
      throw new Error(`Primitive API failed with status ${res.status}: ${errBody}`);
    }

    const data = (await res.json()) as { id?: string };
    const messageId = data?.id || `prim_${Date.now()}`;
